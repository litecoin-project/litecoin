...
The PenguinCoin Core daemon can be configured to act as a trusted "border
router", implementing the penguincoin wire protocol and relay, making
...
The ZeroMQ feature in PenguinCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building penguincoind:
...
For instance:
    ...
    $ penguincoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/penguincoind.tx.raw \
...
These options can also be provided in penguincoin.conf.
...
From the perspective of penguincoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into penguincoind directly. Furthermore, no information is
...
using. PenguinCoind appends an up-counting sequence number to each

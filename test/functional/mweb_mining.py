...
class MWEBMiningTest(LitecoinTestFramework):
    ...
    def run_test(self):
        ...
        node.generate(FIRST_MWEB_HEIGHT - 1, invalid_call=False)
        ...
        assert_raises_rpc_error(
            -8,
            "getblocktemplate must be called with the segwit & mweb rule set",
        ...
        first_mweb_block = node.generate(1, invalid_call=False)[0]
        first_hogex = get_hogex_tx(node, first_mweb_block)
        assert first_hogex is not None
        ...
        node.generatetoaddress(1, node.get_deterministic_priv_key().address, invalid_call=False)
        gbt = node.getblocktemplate(NORMAL_GBT_REQUEST_PARAMS)
        next_height = int(gbt["height"])

        mweb_header = get_mweb_header(node)
        assert mweb_header is not None
    ...
    def test_noncanonical_kernel_features_rejected(self, node):
        ...
        kernel = next(kernel for kernel in mweb_block.body.mweb_kernels if not kernel.features & 4)
    ...
    def mine_many_pegins(self):
        ...
        funder.generate(1, invalid_call=False)
        self.sync_all()
        ...
        block_hash = miner.generate(1, invalid_call=False)[0]
        block = miner.getblock(block_hash)
        hogex = get_hogex_tx(miner, block_hash)
        assert hogex is not None
    ...
    def mine_many_pegouts(self):
        ...
        block_hash = spender.generate(1, invalid_call=False)[0]
    ...
    def mine_after_stale_mweb_spend_reorg(self):
        ...
        miner.generate(1, invalid_call=False)
        self.sync_all()
        ...
        pegin_block = miner.generate(1, invalid_call=False)[0]
        ...
        fork_blocks = fork_miner.generate(2, invalid_call=False)
        ...
        mined_hash = miner.generate(1, invalid_call=False)[0]

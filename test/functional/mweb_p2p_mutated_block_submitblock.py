...
"""Verify a mutated P2P MWEB body does not poison its PoW block hash."""
...
class MWEBP2PMutatedBlockSubmitBlockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        node0.generatetoaddress(1, miner0.getnewaddress(), invalid_call=False)
        self.sync_blocks()
        ...
        submit_hashes = node2.generatetoaddress(3, miner2.getnewaddress(), invalid_call=False)
        ...
        valid_parent_hash = node1.generatetoaddress(1, miner1.getnewaddress(), invalid_call=False)[0]
        valid_child_hash = node1.generatetoaddress(1, miner1.getnewaddress(), invalid_call=False)[0]
        ...
        mutated_inputs = mutated_parent.mweb_block.body.mweb_inputs
        assert_equal(len(mutated_inputs), 2)
        assert mutated_inputs[0].output_commit != mutated_inputs[1].output_commit
        mutated_inputs[0].output_commit, mutated_inputs[1].output_commit = (
            mutated_inputs[1].output_commit,
            mutated_inputs[0].output_commit,
        )
        ...
        submit_results = [node0.submitblock(block) for block in submit_blocks]
        assert_equal(submit_results, ["inconclusive", "inconclusive", None])

...
class CompactBlockPeer(P2PInterface):
    ...
    def send_compact_block(self, block):
        ...
        self.send_message(msg_cmpctblock(compact_block.to_p2p()))
...
class MWEBP2PMutatedCompactBlockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        valid_hash = source.generate(1, invalid_call=False)[0]
        ...
        mutated_hash = source.generate(1, invalid_call=False)[0]
        valid_block = FromHex(CBlock(), source.getblock(mutated_hash, 0))
        valid_block.rehash()

        mutated_block = copy.deepcopy(valid_block)
        inputs = mutated_block.mweb_block.body.mweb_inputs
        assert_equal(len(inputs), 2)
        assert inputs[0].output_pk != inputs[1].output_pk
        inputs[0].output_pk, inputs[1].output_pk = (
            inputs[1].output_pk,
            inputs[0].output_pk,
        )

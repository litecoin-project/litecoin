...
def swap_commitments(block):
    ...
    inputs = mutated.mweb_block.body.mweb_inputs
    assert_equal(len(inputs), 2)
    assert inputs[0].output_commit != inputs[1].output_commit
    inputs[0].output_commit, inputs[1].output_commit = (
        inputs[1].output_commit,
        inputs[0].output_commit,
    )
...
def input_commitments(block):
    return [mweb_input.output_commit for mweb_input in block.mweb_block.body.mweb_inputs]
...
class MWEBNewPoWValidBlockTest(BitcoinTestFramework):
    ...
    def fund_two_spenders(self, source):
        ...
        source.generate(1, invalid_call=False)
    ...
    def run_test(self):
        ...
        sender.block_store[valid_block.sha256] = valid_block
        sender.send_message(msg_inv([CInv(MSG_BLOCK, valid_block.sha256)]))
        self.wait_until(lambda: valid_block.sha256 in sender.getdata_requests, timeout=30)

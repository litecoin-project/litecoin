...
class MWEBP2PTest(BitcoinTestFramework):
    ...
    def test_compact_block_versions_after_mweb(self, node):
        ...
        block_hash = int(node.generate(1, invalid_call=False)[0], 16)
        for version, peer in peers:
            ...
            with p2p_lock:
                if version == 3:
                    ...
                    assert block_hash not in peer.compact_blocks
    ...
    def test_inactive_chain_requests(self, node):
        ...
        stale_hash = node.generate(1, invalid_call=False)[0]
        node.invalidateblock(stale_hash)
        active_hash = node.generatetoaddress(2, node.getnewaddress(), invalid_call=False)[-1]
    ...
    def run_test(self):
        ...
        node.generate(101, invalid_call=False)
        self.sync_all()
        ...
        pre_mweb_block_hash = node.generate(remaining_premweb_blocks, invalid_call=False)[-1]
        self.sync_all()
        ...
        post_mweb_block_hash = block_source.generate(1, invalid_call=False)[0]
        ...
        post_mweb_block_hash2 = node.generate(1, invalid_call=False)[0]

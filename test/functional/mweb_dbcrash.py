...
class MWEBDBCrashTest(LitecoinTestFramework):
    ...
    def run_test(self):
        ...
        self.generatetoaddress(node, 1, miner.getnewaddress(), sync_fun=self.no_op)
        ...
        crash_block_hash = self.generatetoaddress(node, 1, miner.getnewaddress(), sync_fun=self.no_op)[0]
        crash_block = from_hex(CBlock(), node.getblock(crash_block_hash, 0))
        assert_equal(len(crash_block.mweb_block.body.mweb_inputs), 1)
        ...
        post_replay_block_hash = self.generatetoaddress(node, 1, miner.getnewaddress(), sync_fun=self.no_op)[0]
        post_replay_block = from_hex(CBlock(), node.getblock(post_replay_block_hash, 0))
        assert_equal(len(post_replay_block.mweb_block.body.mweb_inputs), 1)

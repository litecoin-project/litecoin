...
void test_bulletproof_rangeproof(size_t nbits, size_t expected_size, const secp256k1_bulletproof_generators *gens) {
    ...
    CHECK(secp256k1_bulletproof_rangeproof_prove_impl(&ctx->error_callback, scratch, proof, &plen, NULL, NULL, nbits, &v, NULL, &blind, &commitp, 1, &value_gen[0], gens, nonce, nonce, NULL, 0, 0, NULL) == 1);
    CHECK(plen == expected_size);
    nonce[0] ^= 1;
    CHECK(secp256k1_bulletproof_rangeproof_prove_impl(&ctx->error_callback, scratch, proof2, &plen, NULL, NULL, nbits, &v, NULL, &blind, &commitp, 1, &value_gen[1], gens, nonce, nonce, NULL, 0, 0, NULL) == 1);
    CHECK(plen == expected_size);
    nonce[0] ^= 2;
    CHECK(secp256k1_bulletproof_rangeproof_prove_impl(&ctx->error_callback, scratch, proof3, &plen, NULL, NULL, nbits, &v, NULL, &blind, &commitp2, 1, &value_gen[2], gens, nonce, nonce, NULL, 0, 0, NULL) == 1);
    CHECK(plen == expected_size);
    nonce[0] ^= 3;
    /* Verify once */
...
void test_bulletproof_rangeproof_aggregate(size_t nbits, size_t n_commits, size_t expected_size, const secp256k1_bulletproof_generators *gens) {
    ...
    CHECK(secp256k1_bulletproof_rangeproof_prove_impl(&ctx->error_callback, scratch, proof, &plen, NULL, NULL, nbits, v, NULL, blind, commitp, n_commits, &value_gen, gens, nonce, nonce, NULL, 0, 0, NULL) == 1);
    CHECK(plen == expected_size);

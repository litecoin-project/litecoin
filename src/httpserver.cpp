...
static void http_request_cb(struct evhttp_request* req, void* arg)
{
    ...
    if (i != iend) {
        std::unique_ptr<HTTPWorkItem> item(new HTTPWorkItem(std::move(hreq), path, [fn = i->handler](HTTPRequest* req, const std::string& path_inner) {
            std::string err_msg;
            try {
                return fn(req, path_inner);
            } catch (const std::exception& e) {
                LogPrintf("Unexpected error while processing request for '%s'. Error msg: '%s'\n", req->GetURI(), e.what());
                err_msg = e.what();
            } catch (...) {
                LogPrintf("Unknown error while processing request for '%s'\n", req->GetURI());

...
struct Block {
    ...
    {
        return IsNull() ? std::optional<CAmount>(0) : m_block->GetTotalFee();
    }

    std::optional<CAmount> GetSupplyChange() const noexcept
    {
        return IsNull() ? std::optional<CAmount>(0) : m_block->GetSupplyChange();
    }
    ...
    std::vector<mw::Hash> GetSpentIDs() const
    {
        ...
        for (const mw::Input& input : m_block->GetInputs()) {
...
struct Tx {
    ...
    std::set<mw::Hash> GetSpentIDs() const noexcept
    {
        ...
        for (const mw::Input& input : m_transaction->GetInputs()) {
    ...
    std::vector<mw::Kernel> GetKernels() const noexcept
    {
        if (IsNull()) {
            return std::vector<mw::Kernel>{};
        }

        return m_transaction->GetKernels();
    }
    ...
    std::optional<CAmount> GetFee() const noexcept
    {
        return IsNull() ? std::optional<CAmount>(0) : m_transaction->GetTotalFee();
    }

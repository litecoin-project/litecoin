...
// Converts an EConsensusError into a lowercase reject-reason suffix,
// e.g. UTXO_MISSING -> "utxo-missing".
static std::string ConsensusErrorRejectSuffix(const EConsensusError type)
{
    std::string str = ConsensusErrorString(type);
    for (char& c : str) {
        c = (c == '_') ? '-' : ToLower(c);
    }
    return str;
...
bool Node::ConnectBlock(const CBlock& block, const Consensus::Params& consensus_params, const ChainstateManager& chainman, const CBlockIndex* pindexPrev, CBlockUndo& blockundo, mw::CoinsViewCache& mweb_view, BlockValidationState& state)
{
    ...
    if (!ContextualCheckBlock(block, consensus_params, chainman, pindexPrev, state)) {
        return false;
    }
    ...
    if (!block.mweb_block.IsNull()) {
        ...
        try {
            ...
            // The canonical block hash commits to the MWEB header, but not the
            // full body. A different body can therefore cause any stateful
            // ApplyBlock failure without changing the proof-of-work hash.
            return state.Invalid(BlockValidationResult::BLOCK_MUTATED, strprintf("mweb-connect-failed-%s", ConsensusErrorRejectSuffix(e.GetType())), strprintf("MWEB::Node::ConnectBlock(): Failed to connect MWEB block: %s", e.what()));
        } catch (const std::exception& e) {
            // Not a consensus rejection: an unexpected (system) error prevented the
            // block from being applied. Treat the block as mutated so it is neither
            // marked permanently invalid nor blamed on a peer.
...
bool Node::CheckTransaction(const CTransaction& tx, TxValidationState& state)
{
    ...
    if (tx.HasMWEBTx()) {
        if (const auto tx_error = tx.mweb_tx.m_transaction->Validate()) {
            return state.Invalid(TxValidationResult::TX_WITNESS_MUTATED, strprintf("bad-mweb-txn-%s", ConsensusErrorRejectSuffix(*tx_error)));
        }
    }

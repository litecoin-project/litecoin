...
public:
    ...
    void SaveToWallet(const std::vector<mw::WalletCoin>& coins) EXCLUSIVE_LOCKS_REQUIRED(m_pWallet->cs_wallet);
    ...
    void SaveStagedCoinsToWallet(const std::set<mw::Hash>& output_ids) EXCLUSIVE_LOCKS_REQUIRED(m_pWallet->cs_wallet);

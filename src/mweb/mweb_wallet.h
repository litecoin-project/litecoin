...
public:
    ...
    // Rewinds and persists metadata for every MWEB output in the transaction.
    void RewindOutputs(const CTransaction& tx) EXCLUSIVE_LOCKS_REQUIRED(m_pWallet->cs_wallet);

    // Rewinds and persists output metadata. Returns true only when an owned coin was successfully saved.
    bool RewindOutput(const mw::Output& output) EXCLUSIVE_LOCKS_REQUIRED(m_pWallet->cs_wallet);
    ...
    // Associates a legacy coin with the active keychain only when its stored
    // address proves that the active scan secret was used to create it.
    bool SetActiveMasterScanKeyId(mw::WalletCoin& coin) const EXCLUSIVE_LOCKS_REQUIRED(m_pWallet->cs_wallet);

        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[3].isNull()) {
        ...
        if (!parsed) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Cannot create MWEB multisig addresses");
        }
        output_type = parsed.value();
    }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    std::unique_ptr<SigningProvider> provider = pwallet->GetSolvingProvider(address);

    isminetype mine = pwallet->IsMine(dest);
    ret.pushKV("ismine", bool(mine & ISMINE_SPENDABLE));

    if (provider) {
        auto inferred = InferDescriptor(address, *provider);
    ...
    const auto& spk_mans = pwallet->GetScriptPubKeyMans(address);
    ...
    ret.pushKV("ischange", AddressIsChange(*pwallet, address));

    if (spk_man) {
        if (const std::unique_ptr<CKeyMetadata> meta = spk_man->GetMetadata(dest)) {
            ret.pushKV("timestamp", meta->nCreateTime);
            if (meta->has_key_origin) {
                ret.pushKV("hdkeypath", WriteHDKeypath(meta->key_origin.hdkeypath));

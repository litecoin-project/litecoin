...
inline std::string ConsensusErrorString(const EConsensusError type)
{
    switch (type)
    {
...
public:
    ValidationException(const EConsensusError& type, const std::string& function)
        : LTCException("ValidationException", GetMessage(type), function), m_type(type)
    {

    }

    EConsensusError GetType() const noexcept { return m_type; }

private:
    static std::string GetMessage(const EConsensusError& type)
    {
        return StringUtil::Format("Consensus Error: {}", ConsensusErrorString(type));
    }

    EConsensusError m_type;
};

...
public:
    ...
    void Commit(const FilePath& new_path)
    {
        ...
        // Build, sync, and map the new file before adopting it, so that a
        // failure at any step leaves this object consistent: still mapped to
        // the old (untouched) file, with the pending buffer retained.
        m_mmap.Unmap();
        try {
            File new_file(new_path);
            if (m_file.GetPath().ToString() != new_path.ToString()) {
                m_file.CopyTo(new_path);
            }

            if (m_fileSize != m_bufferIndex || !m_buffer.empty()) {
                new_file.Write(m_bufferIndex, m_buffer, true);
            }

            new_file.Commit();

            MemMap new_mmap{ new_file };
            new_mmap.Map();
            ...
            // All writes went to the new path; restore the old mapping so
            // reads remain valid, and let the caller treat the flush as fatal.
            m_mmap.Map();
            throw;
        }
    }

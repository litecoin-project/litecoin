...
BOOST_AUTO_TEST_CASE(ValidateState)
{
    ...
    // State is valid
    BOOST_REQUIRE(!KernelSumValidator::ValidateState(utxo_commitments, kernels, total_offset));
...
BOOST_AUTO_TEST_CASE(ValidateForBlock)
{
    ...
    BOOST_REQUIRE(!KernelSumValidator::ValidateForTx(*tx1)); // Sanity check
    ...
    mw::Transaction::CPtr tx2 = test::TxBuilder()
        ...
        .Build().GetTransaction();
    BOOST_REQUIRE(!KernelSumValidator::ValidateForTx(*tx2)); // Sanity check
    ...
    mw::Transaction::CPtr tx3 = test::TxBuilder()
        ...
        .Build().GetTransaction();
    BOOST_REQUIRE(!KernelSumValidator::ValidateForTx(*tx3)); // Sanity check
    ...
    BOOST_REQUIRE(!KernelSumValidator::ValidateForBlock(pAggregated->GetBody(), total_offset, prev_total_offset));
}
...
BOOST_AUTO_TEST_CASE(ValidateForBlockWithoutBuilder)
{
    ...
    kernels.push_back(mw::Kernel::Create(excess, std::nullopt, fee, std::nullopt, std::vector<PegOutCoin>{}, std::nullopt, std::vector<uint8_t>{}));
...
BOOST_AUTO_TEST_CASE(ValidateForTx)
{
    ...
    mw::Transaction::CPtr tx1 = test::TxBuilder()
        ...
        .AddPlainKernel(500'000)
        .Build().GetTransaction();
    BOOST_REQUIRE(!KernelSumValidator::ValidateForTx(*tx1));
    ...
    mw::Transaction::CPtr tx2 = test::TxBuilder()
        ...
        .AddPeginKernel(8'000'000)
        .Build().GetTransaction();
    BOOST_REQUIRE(!KernelSumValidator::ValidateForTx(*tx2));
    ...
    mw::Transaction::CPtr tx3 = test::TxBuilder()
        ...
        .AddPegoutKernel(4'500'000, 500'000)
        .Build().GetTransaction();
    BOOST_REQUIRE(!KernelSumValidator::ValidateForTx(*tx3));
...
BOOST_AUTO_TEST_CASE(ValidateForTx_AllowsPointAtInfinity)
{
    ...
    mw::Input input = mw::Input::Create(
    ...
    BOOST_REQUIRE(!KernelSumValidator::ValidateForTx(*tx));
    // The point at infinity is only tolerated for transaction (policy) validation.
    // For block validation, the unserializable sum is a consensus error.
    BOOST_REQUIRE(

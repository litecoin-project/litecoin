...
bool CCoinsViewDB::HaveCoin(const AnyOutputID& output_id) const {
    if (output_id.IsMWEB()) {
...
bool CCoinsViewDB::GetMWEBCoin(const mw::Hash& output_id, mw::Coin::CPtr& coin) const {
    assert(GetMWEBView() != nullptr);
    coin = GetMWEBView()->GetCoin(output_id);
...
bool CCoinsViewDB::BatchWrite(CCoinsMap &mapCoins, const uint256 &hashBlock, const mw::CoinsViewCache::Ptr& derivedView) {
    if (m_mweb_flush_failed) {
        LogPrintf("%s: refusing to write coin database after a failed MWEB flush\n", __func__);
        return false;
    }
    ...
    // MWEB: Flushes MWEB coins & MMRs. This writes and syncs the new MMR/leafset
    // file generation now; the corresponding leveldb entries (coins, leaves,
    // MMRInfo) go into this final batch so they commit atomically with
    // DB_BEST_BLOCK. On failure the batch is discarded, leaving leveldb at the
    // old MMRInfo generation (any new-generation files are orphans that later
    // flushes overwrite). The failure is latched because the coin updates
    // erased from mapCoins above are now neither in memory nor on disk: a
    // subsequent flush (e.g. at shutdown) must not mark the database
    // consistent. The caller aborts the node and restart recovers, replaying
    // from DB_HEAD_BLOCKS if a partial batch was committed.
    try {
        derivedView->Flush(batch.get());
    } catch (const std::exception& e) {
        LogPrintf("%s: MWEB flush failed: %s\n", __func__, e.what());
        m_mweb_flush_failed = true;
        return false;
    }
